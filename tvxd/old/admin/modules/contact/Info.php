...
$adm_modname = 'contact';
	...
	if($db->sql_numrows($result) > 0) {
		...
		while(list($id, $pid, $pid_name, $title, $ctname, $time, $status) = $db->sql_fetchrow($result)) {
			...
			switch($status) {
				case 2: $status = "<a href=\"modules.php?f=contact&do=view_ct&id=$id\" info=\""._PROCESSED."\"><i class=\"fa fa-check fa-white fa-lg\"></i></a>"; break;
				case 1: $status = "<a href=\"modules.php?f=contact&do=view_ct&id=$id\" info=\""._PROCESSING."\"><i class=\"fa fa-pencil fa-white fa-lg\"></i></a>"; break;
				case 0: $status = "<a href=\"modules.php?f=contact&do=view_ct&id=$id\" info=\""._NOPROCESS."\"><i class=\"fa fa-remove fa-red fa-lg\"></i></a>"; break;
			}
			echo "<tr>\n";
