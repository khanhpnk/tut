...
if (defined('iS_ADMIN')) {
	...
	$result = $db->sql_query("SELECT  id, pid, title, pid_name, ctname, time, status  FROM ".$prefix."_contact WHERE alanguage='$currentlang' ORDER BY time DESC LIMIT $offset, $perpage");
	if($db->sql_numrows($result) > 0) {
		...
		echo "<td class=\"row3sd\">Tiêu đề</td>\n";
		...
		$i =0;
		while(list($id, $pid, $title, $pid_name, $ctname, $time, $status) = $db->sql_fetchrow($result)) {

			switch($status) {
				case 0: $status_o = "<i class=\"fa fa-minus-circle fa-lg\"></i>";  break;
				case 1: $status_o = "<i class=\"fa fa-ellipsis-h\"></i>"; break;
				case 2: $status_o = "<i class=\"fa fa-check-circle fa-lg fa-green\"></i>"; break;
			}
			...
			echo "</td>\n";
			echo "<td align=\"left\" class=\"row1\">$title</td>\n";
			...
			echo "<td align=\"center\" width=\"30\" class=\"row3\"><a href=\"?f=$adm_modname&do=view_ct&id=$id\" info=\""._VIEW."\"><i class=\"fa fa-list fa-lg\"></i></a></td>\n";
			if($ajax_active == 1) {
				echo "<td align=\"center\" width=\"30\" class=\"row1\"><a href=\"?f=$adm_modname&do=delete_ct&id=$id\" title=\""._DELETE."\" onclick=\"return aj_base_delete($id,'$adm_modname','"._DELETEASK1."','delete_ct','');\"><i class=\"fa fa-trash-o fa-lg\"></i></td>\n";
			} else {
				echo "<td align=\"center\" width=\"30\" class=\"row1\"><a href=\"?f=$adm_modname&do=delete_ct&id=$id\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK1."');\"><i class=\"fa fa-trash-o fa-lg\"></i></td>\n";
			}
			echo "</tr>\n";
			$i ++;
		}
