...
if ($db->sql_numrows() > 0) {
	...
	while (list($id, $group, $tname, $money, $temail, $phone, $address, $activationCode, $active, $registrationTime, $recoverCode, $loginAttempt, $unblockCode) = $db->sql_fetchrow($result)) {
		if($ajax_active == 1) {	
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&stat=0\" title=\""._DEACTIVATE."\" onclick=\"return aj_base_status($id,0,'$adm_modname','','');\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&stat=1\" title=\""._ACTIVE."\" onclick=\"return aj_base_status($id,1,'$adm_modname','','');\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}
		} else {
			switch($active) {
				case 1: $active = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&stat=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>"; break;
				case 0: $active = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&stat=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>"; break;
			}	
		}
		if(is_null($activationCode))
		{
			$activation ="<a href=\"modules.php?f=$adm_modname&do=status&id=$id&active=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>";
		}
		else
		{
			$activation = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&active=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>";
		}
		if(is_null($unblockCode))
		{
			$unblock ="<a href=\"modules.php?f=$adm_modname&do=status&id=$id&block=0\" info=\""._DEACTIVATE."\"><img border=\"0\" src=\"images/view.png\"></a>";
		}
		else
		{
			$unblock = "<a href=\"modules.php?f=$adm_modname&do=status&id=$id&block=1\" info=\""._ACTIVE."\"><img border=\"0\" src=\"images/viewo.png\"></a>";
		}
		if (($i % 2) == 1) $css = "row1";
		else $css ="row3";
