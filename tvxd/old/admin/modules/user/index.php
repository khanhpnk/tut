...
$s_phone = isset($_GET["s_phone"]) ? $_GET["s_phone"] : '';
$s_time = isset($_GET["s_time"]) ? $_GET["s_time"] : "";
$s_title = isset($_GET["s_title"]) ? $_GET["s_title"] : "";
$s_quantity=isset($_GET["s_quantity"]) ? $_GET["s_quantity"] : 15;
...
<form action="" name="frmtool" method="get">
	...
	<input type="text" id="s_phone" value="" name="s_phone"  style="width: 120px" />
	...
	<input type="text" id="to" name="to"  style="width: 80px"/>
		...
		<option value="3">Không khóa</option>
		<option value="4">Nhiều EP nhất</option>
		<option value="5">ít EP nhất</option>
		<option value="41">Nhiều VP nhất</option>
		<option value="51">ít VP nhất</option>
		<option value="6">Giao dịch nhiều nhất</option>
		<option value="7">Giao dịch Ít nhất</option>
		<option value="8">Đăng ký mới nhất</option>
		<option value="9">Đăng ký lâu nhất</option>
	</select>
	...
	<a href="/admin/modules.php?f=user&do=export" title="" target="_blank">
		<input type="button" name="" class="button2" value="Export Email" />
	</a>
</form>
...
echo "<td align=\"center\" width=\"100\" class=\"row1sd\"><b>EP</b></td>\n";
echo "<td align=\"center\" width=\"100\" class=\"row1sd\"><b>VP</b></td>\n";
echo "<td align=\"center\" width=\"80\" class=\"row1sd\"><b>Giao dịch</b></td>\n";
echo "<td align=\"center\" width=\"80\" class=\"row1sd\"><b>K.Hoạt</b></td>\n";
...
$result = $db->sql_query("SELECT id, group_id, fullname, money, mep, email, phone, address, activationCode, actives, registrationTime, recoverCode, loginAttempt, unblockCode FROM {$prefix}_user $where LIMIT $offset, $s_quantity");
...
if ($db->sql_numrows() > 0) {
	$i = 0;
	while (list($id, $group, $tname, $money, $mep, $temail, $phone, $address, $activationCode, $active, $registrationTime, $recoverCode, $loginAttempt, $unblockCode) = $db->sql_fetchrow($result)) {
		if($ajax_active == 1) {	
			switch($active) {
		...
		echo "<td align=\"right\" class=\"row1\">".bsVndDot($money)." <a class=\"fancybox50 fancybox.iframe\"  href=\"modules.php?f=$adm_modname&do=addmoney&id=$id\"><img border=\"0\" src=\"images/money.png\"  info=\"Cộng trừ tiền tài khoản\"></a></td>\n";
		echo "<td align=\"right\" class=\"row1\">".bsVndDot($mep)." <a class=\"fancybox50 fancybox.iframe\"  href=\"modules.php?f=$adm_modname&do=addmep&id=$id\"><img border=\"0\" src=\"images/money.png\"  info=\"Cộng trừ tiền tài khoản\"></a></td>\n";
