...
echo "</table></form>";
echo "</div>";
$resultcat = $db->sql_query("SELECT catid, title, active, weight, counts, startid, onhome, homelinks, parent FROM {$prefix}_news_cat WHERE alanguage='$currentlang' ORDER BY weight");
if($db->sql_numrows($resultcat) > 0) {
...
echo "	<tr>\n";
		...
		echo "<td align=\"center\" class=\"row1\"><a href=\"?f=$adm_modname&do=delete&id=$catid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK."');\"><i class=\"fa fa-trash-o fa-lg\"></i></td>\n";
		echo "</tr>\n";
echo childcat($catid);
}
...
function childcat($catid, $text="&nbsp;&nbsp;") {
	...
	if($db->sql_numrows($result) > 0 ) {
		...
		$treeTemp .= "<td align=\"center\" class=\"row1\"><a href=\"?f=$adm_modname&do=delete&id=$catid\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK."');\"><i class=\"fa fa-trash-o fa-lg\"></i></td>\n";
		$treeTemp .= "</tr>\n";
