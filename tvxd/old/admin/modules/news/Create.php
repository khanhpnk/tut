...
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		$query = "INSERT INTO $insertIntoTable (id, catid, title, permalink, alanguage, hometext, bodytext, seo_title, seo_description, seo_keyword, seo_tag, fattach, news_type, images, imgtext, active, source, imgshow, othershow, image_highlight, hits, nstart, special, user_id";
		if ($timed) $query .= ', timed';
		else $query .= ', time';
		$query .= ") VALUES ($id, $catid, '$title', '$permalink', '$currentlang', '$hometext', '$bodytext', '$title_seo', '$description_seo', '$keyword_seo', '$tag_seo', '$fattach', '$news_type', '$images', '$imgtext', $active, '$source', $imgshow, '$othershow', $highlight, 0, $startid, $special,$user_id";
...
$result_cat = $db->sql_query("SELECT catid, title FROM ".$prefix."_news_cat WHERE catid IN (SELECT news FROM ".$prefix."_news_permission WHERE admingroup=(SELECT permission FROM ".$prefix."_admin WHERE adacc='$admin_ar[0]')) AND parent='0' and alanguage='$currentlang' ORDER BY weight");
