...
if($permission!=2){
	$where .= " AND active=0 AND user_id=(SELECT id FROM ".$prefix."_admin WHERE adacc='$admin_ar[0]')";
}
...
<form action="" name="frmtool" method="get">
	...
	<?php
	$resultcat = $db->sql_query("SELECT catid, title FROM {$prefix}_news_cat WHERE catid IN (SELECT news FROM ".$prefix."_news_permission WHERE admingroup=(SELECT permission FROM ".$prefix."_admin WHERE adacc='$admin_ar[0]')) AND parent=0 AND alanguage='$currentlang' ORDER BY weight");
...
echo "<div id=\"pagecontent\">";
	...
	echo "<td class=\"row1sd\" align=\"center\" width=\"120\">"._TIMEUP." <a href=\"?f=".$adm_modname."&sort=3\" info=\""._SORTUP."\"><img border=\"0\" src=\"images/sup.gif\" align=\"absmiddle\"></a> <a href=\"?f=".$adm_modname."&sort=4\" info=\""._SORTDOWN."\"><img border=\"0\" src=\"images/sdown.gif\" align=\"absmiddle\"></a></td>\n";
	...
	while(list($id, $catid, $title, $time, $active, $hits, $nstart, $newsType) = $db->sql_fetchrow($result)) {
		...
		if($ajax_active == 1) {
			if($permission==2){
			switch($active) {
				case 1: $active = "<a href=\"?f=".$adm_modname."&do=status_news&type=$newsType&id=$id&stat=0\" title=\""._DEACTIVATE."\" onclick=\" aj_base_status($id,'0','$adm_modname','status_news',mid); return false;\"><i class=\"fa fa-check-circle fa-lg fa-green\"></i></a>"; break;
				case 0: $active = "<a href=\"?f=".$adm_modname."&do=status_news&type=$newsType&id=$id&stat=1\" title=\""._ACTIVE."\" onclick=\" aj_base_status($id,'1','$adm_modname','status_news',mid); return false;\"><i class=\"fa fa-minus-circle fa-lg fa-red\"></i></a>"; break;
			}
			}
			else
			{
				switch($active) {
				case 1: $active = "<i class=\"fa fa-check-circle fa-lg fa-green\"></i>"; break;
				case 0: $active = "<i class=\"fa fa-minus-circle fa-lg fa-red\"></i>"; break;
			}
			}
			switch($nstart) {
		...
		echo "<td align=\"center\" width=\"30\" class=\"$css\"><a href=\"?f=".$adm_modname."&do=edit_news&type=$newsType&id=$id\" info=\""._EDIT."\"><i class=\"fa fa-pencil-square-o fa-lg fa-blue\"></i></a></td>\n";
		if($ajax_active == 1) {
			echo "<td align=\"center\" width=\"30\" class=\"row3\"><a href=\"?f=".$adm_modname."&do=delete_news&type=$newsType&id=$id\" title=\""._DELETE."\" onclick=\"aj_base_delete($id,'$adm_modname','"._DELETEASK1."','delete_news','');\"><i class=\"fa fa fa-trash-o fa-lg\"></i></td>\n";
		} else {
			echo "<td align=\"center\" width=\"30\" class=\"row3\"><a href=\"?f=".$adm_modname."&do=delete_news&type=$newsType&id=$id\" info=\""._DELETE."\" onclick=\"return confirm('"._DELETEASK1."');\"><i class=\"fa fa-trash-o fa-lg\"></i></td>\n";
		}
		echo "</tr>\n";
		$i++;
	}
