...
$sEmail = $sCEmail = $sAddress = $sPhone = $sName = $error_cpassword = $error_address = $error_phone = $error_password = $error_cemail = $error_name = $error_captcha =$error_email='';
...
//$captcha = new CAPTCHA(6);
	...
	$result = $db->sql_query("SELECT phone FROM ".$prefix."_user WHERE phone='$phone'");
	if($db->sql_numrows($result) > 0) {
		$err = 1;
		$error_phone = "<font  color=\"red\">Điện thoại đã tồn tại!</font>";
	}
	...
	if (!$err) {
		...
		if ($ret == WEBUSER_EMAIL_REGISTERED) {
			...
			//echo "<div class=\"content\" style=\"height:300px; padding-top:50px\" align=\"center\"><b>"._USER_REGISTRATION_SUCCESSFUL."</b><p><b> Mời bạn kiểm tra email <b>".$email."</b> để kích hoạt tài khoản!<br><i>Nếu yêu cầu kích hoạt tài khoản của thuvienxaydung.net không có trong Hộp thư đến (Inbox), vui lòng kiểm tra trong mục Thư rác (Spam).</i></b></p></div>";
			echo "<div class=\"content\" style=\"height:300px; padding-top:50px\" align=\"center\"><b>"._USER_REGISTRATION_SUCCESSFUL."</b>			<p>
<strong>Bước tiếp: Xác minh  tài khoản</strong><br/>
Để xác minh tài khoản đăng ký mời bạn soạn tin nhắn:<br/>

<strong>ON   XMXD1</strong>    gửi 8085 (Phí tin nhắn 500đ/1 tin)<br/>
</p></div>";
...
//echo "<div>
//<p>
//<strong>1.	Đăng kí thành viên</strong><br/>
//Để đăng ký thành viên mời bạn soạn tin nhắn SMS theo cú pháp:<br/>
//<strong>ON    DKXD    TAI_KHOAN</strong> gửi 8085<br/>
//Trong đó:<br/>
//<strong>ON   DKXD</strong> : Cú pháp mặc định<br/>
//<strong>TAI_KHOAN</strong>: Tên tài khoản do bạn đặt tùy ý<br/>
//Ví dụ: <strong>ON   DKXD   nguyenvana</strong>     Gửi 8085<br/>
//Sau khi gửi SMS thành công tổng đài gửi về cho bạn mật khẩu cấp 1 và mật khẩu cấp 2 - mật khẩu cấp 1 dùng để đăng nhập, mật khẩu cấp 2 dùng để giao dịch. Bạn đăng nhập website đổi mật khẩu mới<br/>
//Lưu ý: Tên tài khoản ít nhất 6 ký tự trở lên, một số điện thoại chỉ đăng ký được một tài khoản<br/>
//</p><p>
//<strong>2.	Quên mật khẩu</strong><br/>
//Để lấy lại mật khẩu mời bạn soạn tin nhắn:<br/>
//
//<strong>ON    MKXD</strong>  gửi 8085 ( Lấy lại mật khẩu cấp 1)<br/>
//<strong>ON    MKX2</strong>   gửi 8085 ( Lấy lại mật khẩu cấp 2)<br/>
//(Phí tin nhắn 500đ/1 tin)<br/>
//</p><p>
//<strong>3.	Xác minh  tài khoản</strong><br/>
//Để xác minh tài khoản đăng ký trước ngày 20/12/2015 mời bạn soạn tin nhắn:<br/>
//
//<strong>ON   XMXD1</strong>    gửi 8085 (Phí tin nhắn 500đ/1 tin)<br/>
//</p></div>";
...
<?php dsp_crypt(0,1,$urlsite); ?> <?echo $error_captcha?>
</td>
</tr>
<?php
