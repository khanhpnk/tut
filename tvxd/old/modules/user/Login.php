    ...
    if ($user) {
        ...
        if ($ret == USER_LOGIN_SUCCEEDED) {
            if (!isset($_GET['type']) || ($_GET['type'] != 'quick')) {
                ...
                list($totaldocument) = $db->sql_fetchrow($db->sql_query("SELECT id FROM " . $prefix . "_document WHERE user_id='" . $userInfo['id'] . "'"));
                if ($totaldocument == 0)
                    echo "<meta http-equiv=\"refresh\" content=\"1;url= " . url_sid("index.php") . "\">";
                else
                    echo "<meta http-equiv=\"refresh\" content=\"1;url= " . url_sid("index.php?f=user") . "\">";
                CloseTab();
            ...
            $err_mess = _USER_ACCOUNT_BLOCKED . " Để mở khóa <a href=" . url_sid("index.php?f=$module_name&do=xac_minh") . "> click ở đây</a>";
