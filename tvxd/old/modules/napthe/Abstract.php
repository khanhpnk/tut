...
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	if (!$err) {
		...
		$result = $db->sql_query("SELECT money FROM ".$prefix."_user WHERE id='".$userInfo['id']."'");
			if($db->sql_numrows($result) == 1){
				...
				if($moneyold >= $money)
				{
					...
					updateuserlog($userInfo['id'],"Rút tiền",$moneyold, $money, $moneynew,"-","Rút tiền vào ngân hàng tài khoản ".$txt_accountnumber."");
				}
				else
				{
					die('<script>alert("Tai khoan con '.bsVndDot($moneyold).' VND khong du de thuc hien giao dich");history.go(-1);</script>');
				}
			}
		?>
...
<td class="row1"><input type="password" name="pass2"  id="pass2" size="40"><?echo $error_pass2?>
</td>
</tr>
