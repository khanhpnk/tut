...
if (($key_knp==$TxtKey) && ($key==md5('123.30.243.69')) && (!empty($TxtType)) && (!empty($TxtTransId)) && (!empty($TxtMenhGia)) && $TxtMenhGia!="") {
   // lay thong tin nguoi dung theo transid
   $resultinfo=$db->sql_query("SELECT u.id, u.fullname, u.email, u.money, n.id, n.telecom, n.transid FROM {$prefix}_user AS u,".$prefix."_napthe AS n WHERE u.id=n.userid AND transid='$TxtTransId' AND status=0");
	if($db->sql_numrows($resultinfo) > 0) {
		...
		$db->sql_query($query);
		updateuserlog($uid,'Nạp thẻ', $money_old, $tiennhan, $moneynew, "+","".$ufullname." Trả tiền thẻ trễ $TxtTransId");
