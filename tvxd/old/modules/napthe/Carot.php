...
if( isset($_POST['subup']) && $_POST['subup'] == 1) {
	...
	//}
	if($userInfo['pass2']!=md5($pass2))
	{
		$err = 1;
		$error_pass2 = "<font  color=\"red\">Mật khẩu cấp 2 không đúng!</font>";
	}
	//kiem tra so luong the cao co du thuc giao dich
	$result=$db->sql_query("SELECT id, code, serial FROM {$prefix}_thecao WHERE buy=0 AND menhgia=$menhgia LIMIT $quantity");
	...
	if (!$err) {
		//viet lai phan tinh phan tram cho the
		//neu xac dinh chinh xac loai the cao, thanh vien, menh gia
		$resultpromotion=$db->sql_query("SELECT id, username, thecao, menhgia, promotion, time, active FROM {$prefix}_thecao_promotion WHERE thecao=$catid AND username =".$userInfo['id']." AND menhgia=$menhgia AND active=1 LIMIT 1");
		if($db->sql_numrows($resultpromotion) > 0) {
			...
			//neu xac dinh chinh xac the cao, thanh vien, menh gia thi khong
			$resulttal1=$db->sql_query("SELECT id, username, thecao, menhgia, promotion, time, active FROM {$prefix}_thecao_promotion WHERE thecao=$catid AND menhgia=0 AND username =".$userInfo['id']." AND active=1 LIMIT 1");
			if($db->sql_numrows($resulttal1) >0) {
				list($pmt_id1, $pmt_username1, $pmt_thecao1, $pmt_menhgia1, $pmt_promotion1, $pmt_time1, $pmt_active1) = $db->sql_fetchrow($resulttal1);
				$promotionkm = $pmt_promotion1/100;
			}
			else
			{
				$resulttal2=$db->sql_query("SELECT id, username, thecao, menhgia, promotion, time, active FROM {$prefix}_thecao_promotion WHERE thecao=$catid AND menhgia=$menhgia AND username = 0 AND active=1 LIMIT 1");
				if($db->sql_numrows($resulttal2) >0) {
					list($pmt_id2, $pmt_username2, $pmt_thecao2, $pmt_menhgia2, $pmt_promotion2, $pmt_time2, $pmt_active2) = $db->sql_fetchrow($resulttal2);
					$promotionkm = $pmt_promotion2/100;
				}
				else{
					$resulttal3=$db->sql_query("SELECT id, username, thecao, menhgia, promotion, time, active FROM {$prefix}_thecao_promotion WHERE thecao=$catid AND menhgia= 0 AND username = 0 AND active=1 LIMIT 1");
					if($db->sql_numrows($resulttal3) >0) {
						list($pmt_id3, $pmt_username3, $pmt_thecao3, $pmt_menhgia3, $pmt_promotion3, $pmt_time3, $pmt_active3) = $db->sql_fetchrow($resulttal3);
						$promotionkm = $pmt_promotion3/100;
					}
				}
		...
		//neu ton tai the cao va nguoi dung khuyen mai nhieu muc khac nhau
		...
		list($g_menhgia, $g_giaban) = $db->sql_fetchrow($result);
		{
			...
			elseif($totalorder <= $userInfo['money'])
			{
				...
				if($db->sql_numrows($result) == $quantity) {
					...
					{
						//lay ma the 
						$result=$db->sql_query("SELECT id, catid, code, serial FROM {$prefix}_thecao WHERE buy=0 AND menhgia=$menhgia ORDER BY id ASC LIMIT 1");
						if($db->sql_numrows($result) == 1) {
							list($g_id, $g_catid, $g_code, $g_serial) = $db->sql_fetchrow($result);
							...
							//
							$result_cat=$db->sql_query("SELECT title FROM {$prefix}_thecao_cat WHERE catid=$g_catid");
							if($db->sql_numrows($result_cat) == 1) {
								list($c_title) = $db->sql_fetchrow($result_cat);
							}
							
							updateuserlog($userInfo['id'],"Mua thẻ", $money_old, $price, $money_new,"-","Mua thẻ $c_title code: $code | serial: $serial |  m".$catid.":".$userInfo['id'].":".$menhgia.":".$promotionkm."");
...
?>
<tr>
<td class="row1">Mật khẩu cấp 2:<span class="risk">*</span></td>
<td class="row1"><input type="password" name="pass2"  id="pass2" style="width:250px"><?echo $error_pass2?>
</td>
</tr>
<?php
echo "</tr>\n";
